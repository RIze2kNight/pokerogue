 ...
 * @param globalScene current BattleScene
 */
export function resetSettings() {
  ModSetting.forEach(s => setModSetting(s.key, s.default));
}
 ...
 */
export function setModSetting(setting: string, value: number): boolean {
  ...
  switch (ModSetting[index].key) {
    case SettingKeys.INFINITE_BALLS:
      globalScene.mods.infiniteBalls = ModSetting[index].options[value].value === "On";
      break;
    case SettingKeys.CATCH_TRAINER_POKEMON:
      const trainerValue = parseInt(ModSetting[index].options[value].value);
      globalScene.mods.catchTrainerPokemon = trainerValue > 0;
      globalScene.mods.catchTrainerPokemonRestrictions = trainerValue === 1;
      break;
    case SettingKeys.HIDDEN_ABILITY:
      globalScene.mods.hiddenAbilityModifier = parseInt(ModSetting[index].options[value].value);
      break;
    case SettingKeys.SHINY:
      globalScene.mods.shinyModifier = parseInt(ModSetting[index].options[value].value);
      break;
    case SettingKeys.INFINITE_VOUCHERS:
      globalScene.mods.infiniteVouchers = ModSetting[index].options[value].value === "On";
      break;
    case SettingKeys.WAVE_EGG_HATCH:
      globalScene.mods.overrideEggHatchWaves = ModSetting[index].options[value].value === "On";
      break;
    case SettingKeys.EGG_RARITY:
      ...
      globalScene.mods.overrideEggRarityIndex = eggRarityMult;
      console.log(globalScene.mods.overrideEggRarityIndex);
      break;
    case SettingKeys.EGG_SPECIES_PITTY:
      globalScene.mods.eggSpeciesPitty = parseInt(ModSetting[index].options[value].value);
      break;
    case SettingKeys.CANDY_COST_MULTIPLIER:
      globalScene.mods.candyCostMultiplier = parseFloat(ModSetting[index].options[value].value);
      break;
